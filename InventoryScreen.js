...
import { 
    ...
    AlertTriangle, X as CloseIcon, Edit, Trash2, ArrowRight,
...
const translations = {
    en: {
        title: "Inventory", subTitle: "Government Asset Management",
        add: "Add Asset", search: "Search assets...", 
        name: "Item Name", category: "Category", condition: "Condition",
        quantity: "Quantity", value: "Value (LKR)", serial: "Serial No",
        save: "Save Asset", cancel: "Cancel",
        noData: "No assets found", stats: "View Stats"
    },
    si: {
        title: "ඉන්වෙන්ටරි", subTitle: "රජයේ වත්කම් කළමනාකරණය",
        add: "වත්කමක් එක් කරන්න", search: "වත්කම් සොයන්න...",
        name: "අයිතමයේ නම", category: "වර්ගය", condition: "තත්ත්වය",
        quantity: "ප්‍රමාණය", value: "වටිනාකම (රු.)", serial: "අනුක්‍රමික අංකය",
        save: "සුරකින්න", cancel: "අවලංගු කරන්න",
        noData: "වත්කම් හමු නොවීය", stats: "සංඛ්‍යාලේඛන"
    },
    ta: {
        title: "சரக்கு", subTitle: "அரசு சொத்து மேலாண்மை",
        add: "சொத்தைச் சேர்", search: "சொத்துக்களைத் தேடு...",
        name: "பொருளின் பெயர்", category: "வகை", condition: "நிலை",
        quantity: "அளவு", value: "மதிப்பு (LKR)", serial: "வரிசை எண்",
        save: "சேமி", cancel: "ரத்து",
        noData: "சொத்துக்கள் எதுவும் இல்லை", stats: "புள்ளிவிவரங்கள்"
    }
};
...
export default function InventoryScreen() {
    ...
    const validateAssetForm = () => {
        let errors = {};
        const { itemName, category, quantity, purchaseValue, lastServiceDate, warrantyExpiry } = assetForm;
        
        if (!itemName) errors.itemName = "Asset name is required";
        if (!category) errors.category = "Category is required";
        
        if (!quantity) errors.quantity = "Quantity is required";
        else if (isNaN(quantity) || parseInt(quantity) <= 0) errors.quantity = "Enter a positive number";
        
        if (!purchaseValue) errors.purchaseValue = "Value is required";
        else if (isNaN(purchaseValue) || parseFloat(purchaseValue) < 0) errors.purchaseValue = "Enter a valid amount";
        
        const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
        if (lastServiceDate && !dateRegex.test(lastServiceDate)) {
            errors.lastServiceDate = "Format: YYYY-MM-DD";
        }
        if (warrantyExpiry && !dateRegex.test(warrantyExpiry)) {
            errors.warrantyExpiry = "Format: YYYY-MM-DD";
        }
    ...
    const handleSaveAsset = async () => {
        ...
        try {
            const payload = { 
                ...
                purchaseValue: assetForm.purchaseValue ? parseFloat(assetForm.purchaseValue) : 0 
            };
            ...
            Alert.alert("Error", "Failed to save asset. Please check your connection."); 
        }
    };
    ...
    const validateBorrowForm = () => {
        let errors = {};
        const { requestedBy, requestedByNic, quantityRequested, expectedReturnDate } = borrowForm;
        
        if (!requestedBy) errors.requestedBy = "Full name is required";
        if (!requestedByNic) {
            errors.requestedByNic = "NIC is required";
        } else if (!/^[0-9]{9}[vVxX]$|^[0-9]{12}$/.test(requestedByNic)) {
            errors.requestedByNic = "Invalid NIC format";
        }
        
        if (!quantityRequested) {
            errors.quantityRequested = "Quantity is required";
        } else if (isNaN(quantityRequested) || parseInt(quantityRequested) <= 0) {
            errors.quantityRequested = "Enter a positive number";
        } else if (selectedAsset && parseInt(quantityRequested) > selectedAsset.quantity) {
            errors.quantityRequested = `Only ${selectedAsset.quantity} available in stock`;
        }
        
        const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
        if (!expectedReturnDate) {
            errors.expectedReturnDate = "Return date required";
        } else if (!dateRegex.test(expectedReturnDate)) {
            errors.expectedReturnDate = "Use YYYY-MM-DD format";
        } else {
            const selectedDate = new Date(expectedReturnDate);
            const today = new Date();
            today.setHours(0, 0, 0, 0);
            if (selectedDate < today) {
                errors.expectedReturnDate = "Return date cannot be in the past";
            }
        }
    ...
    const handleBorrowSubmit = async () => {
        ...
        try {
            await api.post('/assets/requests/create', { 
                ...borrowForm, 
                assetId: selectedAsset._id, 
                assetName: selectedAsset.itemName,
                quantityRequested: parseInt(borrowForm.quantityRequested)
            });
            setShowBorrowModal(false); 
            setBorrowErrors({});
            fetchData();
            Alert.alert("Success", "Borrow request submitted for review.");
        } catch (err) { 
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <Modal visible={showAssetModal} animationType="slide" transparent={true}>
                <View style={styles.modalBackdrop}>
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalBody}>
                        <View style={styles.modalHeader}><Text style={styles.modalTitle}>{isPickingItem ? "Select Name" : "Asset Registry"}</Text><TouchableOpacity onPress={() => isPickingItem ? setIsPickingItem(false) : setShowAssetModal(false)} style={styles.closeBtn}><CloseIcon size={20} color="#000" /></TouchableOpacity></View>
                        {isPickingItem ? (
                            ...
                            <ScrollView contentContainerStyle={styles.form}>
                                ...
                                <TouchableOpacity onPress={() => setIsPickingItem(true)} style={[styles.pickerTrigger, formErrors.itemName && styles.errorBorder]}><Text>{assetForm.itemName || "-- Select Asset --"}</Text><ChevronDown size={18} color="#64748b" /></TouchableOpacity>
                                {formErrors.itemName && <Text style={styles.errorTextSmall}>{formErrors.itemName}</Text>}

                                <View style={styles.formRow}>
                                    <View style={{flex:1}}><Text style={styles.label}>Quantity *</Text><TextInput style={[styles.input, formErrors.quantity && styles.errorBorder]} value={assetForm.quantity} onChangeText={v => setAssetForm({...assetForm, quantity:v})} keyboardType="numeric" />{formErrors.quantity && <Text style={styles.errorTextSmall}>{formErrors.quantity}</Text>}</View>
                                ...
                                {formErrors.category && <Text style={styles.errorTextSmall}>{formErrors.category}</Text>}
                                ...
                                <View style={styles.formRow}>
                                    <View style={{flex:1}}><Text style={styles.label}>Value (Rs) *</Text><TextInput style={[styles.input, formErrors.purchaseValue && styles.errorBorder]} value={assetForm.purchaseValue} onChangeText={v => setAssetForm({...assetForm, purchaseValue:v})} keyboardType="numeric" />{formErrors.purchaseValue && <Text style={styles.errorTextSmall}>{formErrors.purchaseValue}</Text>}</View>
                                    <View style={{width:15}} /><View style={{flex:1}}><Text style={styles.label}>Service Date</Text><TextInput style={[styles.input, formErrors.lastServiceDate && styles.errorBorder]} value={assetForm.lastServiceDate} onChangeText={v => setAssetForm({...assetForm, lastServiceDate:v})} placeholder="YYYY-MM-DD" />{formErrors.lastServiceDate && <Text style={styles.errorTextSmall}>{formErrors.lastServiceDate}</Text>}</View>
                                </View>

                                <Text style={styles.label}>Warranty Expiry</Text><TextInput style={[styles.input, formErrors.warrantyExpiry && styles.errorBorder]} value={assetForm.warrantyExpiry} onChangeText={v => setAssetForm({...assetForm, warrantyExpiry:v})} placeholder="YYYY-MM-DD" />
                                {formErrors.warrantyExpiry && <Text style={styles.errorTextSmall}>{formErrors.warrantyExpiry}</Text>}
            ...
            <Modal visible={showBorrowModal} animationType="slide" transparent={true}>
                <View style={styles.modalBackdrop}>
                    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalBody}>
                        <View style={styles.modalHeader}><Text style={styles.modalTitle}>Borrow Asset</Text><TouchableOpacity onPress={() => setShowBorrowModal(false)} style={styles.closeBtn}><CloseIcon size={20} color="#000" /></TouchableOpacity></View>
                        <ScrollView contentContainerStyle={styles.form}>
                            <Text style={styles.label}>Full Name *</Text>
                            <TextInput style={[styles.input, borrowErrors.requestedBy && styles.errorBorder]} value={borrowForm.requestedBy} onChangeText={v => setBorrowForm({...borrowForm, requestedBy:v})} />
                            {borrowErrors.requestedBy && <Text style={styles.errorTextSmall}>{borrowErrors.requestedBy}</Text>}

                            <Text style={styles.label}>NIC Number *</Text>
                            <TextInput style={[styles.input, borrowErrors.requestedByNic && styles.errorBorder]} value={borrowForm.requestedByNic} onChangeText={v => setBorrowForm({...borrowForm, requestedByNic:v})} />
                            {borrowErrors.requestedByNic && <Text style={styles.errorTextSmall}>{borrowErrors.requestedByNic}</Text>}

                            <View style={styles.formRow}>
                                <View style={{flex:1}}>
                                    <Text style={styles.label}>Qty *</Text>
                                    <TextInput style={[styles.input, borrowErrors.quantityRequested && styles.errorBorder]} value={borrowForm.quantityRequested} onChangeText={v => setBorrowForm({...borrowForm, quantityRequested:v})} keyboardType="numeric" />
                                    {borrowErrors.quantityRequested && <Text style={styles.errorTextSmall}>{borrowErrors.quantityRequested}</Text>}
                                </View>
                                <View style={{width:15}} />
                                <View style={{flex:1}}>
                                    <Text style={styles.label}>Return Date *</Text>
                                    <TextInput style={[styles.input, borrowErrors.expectedReturnDate && styles.errorBorder]} value={borrowForm.expectedReturnDate} onChangeText={v => setBorrowForm({...borrowForm, expectedReturnDate:v})} placeholder="YYYY-MM-DD" />
                                    {borrowErrors.expectedReturnDate && <Text style={styles.errorTextSmall}>{borrowErrors.expectedReturnDate}</Text>}
                                </View>
                            </View>
...
const styles = StyleSheet.create({
    ...
    headerActions: { flexDirection: 'row', gap: 12, marginTop: 15 },
    ...
    errorTextSmall: { color: '#ef4444', fontSize: 10, marginTop: 4, fontWeight: '600' }
});
