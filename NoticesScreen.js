...
import { 
  ...
  Clock, X as CloseIcon, Tag, AlertTriangle, Info
...
export default function NoticesScreen() {
    ...
    const handleSave = async () => {
        // Validation
        if (!formData.title || formData.title.length < 5) return Alert.alert("Error", "Valid Title (min 5 chars) is required");
        if (!formData.desc_en && !formData.desc_si && !formData.desc_ta) {
            return Alert.alert("Error", "Please provide a description in at least one language");
        }
...
const styles = StyleSheet.create({
    ...
    deleteBtn: { backgroundColor: '#ef4444', padding: 12, borderRadius: 10, flex: 1, alignItems: 'center' },
    btnText: { color: '#fff', fontWeight: 'bold' },
    langSwitch: { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 10, padding: 2, marginTop: 5 },
