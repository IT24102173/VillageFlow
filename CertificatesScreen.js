...
const translations = {
    ...
    ta: {
        title: "சான்றிதழ்கள்", apply: "புதிய கோரிக்கை", history: "எனது கோரிக்கைகள்",
        type: "வகையைத் தேர்ந்தெடுக்கவும்", for: "விண்ணப்பிப்பது", self: "எனக்கு", family: "குடும்ப உறுப்பினர்",
        child: "குழந்தை (18 வயதுக்கு கீழ்)", upload: "ஆதார ஆவணத்தைப் பதிவேற்றவும்", proceed: "கட்டணத்திற்குத் தொடரவும்",
        historyTitle: "விண்ணப்ப வரலாறு", empty: "விண்ணப்பங்கள் எதுவும் இல்லை",
        status: "நிலை", date: "விண்ணப்பிக்கப்பட்டது", nicLabel: "உறவினரின் NIC",
        payment: "நிர்வாகக் கட்டணம்", total: "மொத்தம் செலுத்த வேண்டியது", payNow: "பாதுகாப்பான கட்டணம்",
        download: "சான்றிதழைப் பதிவிறக்கவும்",
        validation: {
            nic: "உறவினரின் NIC ஐ உள்ளிடவும்",
            relationship: "உறவைக் குறிப்பிடவும்",
            file: "ஆதார ஆவணத்தைப் பதிவேற்றவும்"
        },
        cardValidation: {
...
export default function CertificatesScreen() {
    ...
    const handleScanNIC = async () => {
        if (!permission || !permission.granted) {
            const status = await requestPermission();
            if (!status.granted) {
                Alert.alert("Permission Required", "Camera access is needed to scan NIC");
                return;
            }
        }
        setShowScanner(true);
    ...
    const simulateScan = () => {
        ...
        setTimeout(() => {
            ...
            const mockNIC = "199512345678";
            setNic(mockNIC);
            setNicDetails(validateNIC(mockNIC));
            Alert.alert("Scan Successful", `Extracted NIC: ${mockNIC}`);
        }, 3000);
    };
    ...
    const handleDownload = async (item) => {
        try {
            setLoading(true);
            const verifyUrl = `https://villageflow.onrender.com/api/certificates/verify/${item._id || item.id}`;
            ...
            const htmlContent = `
                ...
                <head>
                    <style>
                        ...
                        .seal-container { display: flex; flex-direction: column; align-items: center; gap: 10px; }
                        .seal { width: 80px; height: 80px; border: 4px double #800000; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #800000; font-weight: bold; transform: rotate(-15deg); font-size: 8px; text-align: center; opacity: 0.6; }
                        .qr-code { width: 100px; height: 100px; border: 1px solid #e2e8f0; padding: 5px; border-radius: 8px; }
                        ...
                        .verify-text { font-size: 10px; color: #94a3b8; text-align: center; margin-top: 5px; }
                    </style>
                </head>
                <body>
                    <div class="border">
    ...
    const validateForm = () => {
        ...
        if (applyFor === 'Family') {
            ...
            else if (!(nic.length === 10 || nic.length === 12)) newErrors.nic = "Invalid NIC format (10 or 12 chars)";
        ...
        if (errorKeys.length > 0) {
            Alert.alert("Validation Error", newErrors[errorKeys[0]]);
            setErrors(newErrors);
            return false;
        }
        setErrors({});
    ...
    const handleApply = () => {
        ...
        // 3-Month Duplicate Check
        const threeMonthsAgo = new Date();
        threeMonthsAgo.setMonth(threeMonthsAgo.getMonth() - 3);

        const targetNic = applyFor === 'Self' ? user.nic : nic;
        
        const recentlyApplied = history.find(app => 
            app.certificateType === certType && 
            app.nic === targetNic &&
            new Date(app.createdAt) > threeMonthsAgo
        );

        if (recentlyApplied && !editingId) {
            Alert.alert(
                "Duplicate Request",
                "You have already requested this certificate within the last 3 months. Please wait until the period ends.",
                [{ text: "OK" }]
            );
            return;
        }

        const existingPending = history.find(app => 
            app.certificateType === certType && 
            app.status === 'Pending' && 
            app.nic === targetNic
        );
        
        if (existingPending && !editingId) {
            Alert.alert(
        ...
        if (editingId) {
            // If other fields changed but NOT the certificate type -> No Payment
            if (certType === originalCertType) {
                confirmAndSubmit();
                return;
            } else {
                // If certificate type changed -> Must Pay again
                Alert.alert(
                    "Type Changed",
                    "Changing the certificate type requires a new administrative fee. Your previous payment for the old type will be refunded automatically.",
                    [
                        { text: "Cancel", style: "cancel" },
                        { text: "Continue to Payment", onPress: () => setPaymentMode(true) }
                    ]
                );
                return;
            }
    ...
    const confirmAndSubmit = async () => {
        ...
        try {
            ...
            // Only send nicImage for new applications as the update endpoint doesn't support it
            if (nicFile && !editingId) {
                formData.append('nicImage', {
                    uri: nicFile.uri,
                    name: nicFile.name || 'nic_image.jpg',
                    type: nicFile.mimeType || 'image/jpeg'
                });
            }
    ...
    const StatusTracker = ({ status }) => {
        const steps = [
            ...
            { label: status === 'Rejected' ? 'Denied' : 'Done', icon: status === 'Approved' ? <Check size={12} color="#fff" /> : status === 'Rejected' ? <CloseIcon size={12} color="#fff" /> : <Clock size={12} color="#fff" /> }
        ];
        ...
        return (
            <View key={item._id || item.id} style={styles.historyCard}>
                <View style={styles.historyHeader}>
                    <View style={styles.historyIconBox}><Award size={22} color="#800000" /></View>
                    <View style={{ flex: 1 }}>
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
                    ...
                    paymentMode ? (
                        <View style={styles.paymentCard}>
                            ...
                            <View style={styles.payDetails}>
                                ...
                                {paymentMethod === 'Card' && (
                                    <View style={styles.cardForm}>
                                        <TextInput style={[styles.input, errors.cardNumber && styles.errorBorder]} placeholder="Card Number (16 digits)" keyboardType="numeric" maxLength={19} value={cardData.number} onChangeText={v => setCardData({...cardData, number: v.replace(/\D/g, '').replace(/(.{4})/g, '$1 ').trim()})} />
                                        <View style={{flexDirection: 'row', gap: 10, marginTop: 10}}>
                                            <TextInput style={[styles.input, {flex: 1}, errors.cardExpiry && styles.errorBorder]} placeholder="MM/YY" maxLength={5} value={cardData.expiry} onChangeText={v => setCardData({...cardData, expiry: v})} />
                                            <TextInput style={[styles.input, {flex: 1}, errors.cardCvv && styles.errorBorder]} placeholder="CVV" maxLength={3} secureTextEntry value={cardData.cvv} onChangeText={v => setCardData({...cardData, cvv: v})} />
                                        </View>
                                        <TextInput style={[styles.input, {marginTop: 10}, errors.cardName && styles.errorBorder]} placeholder="Cardholder Name" value={cardData.name} onChangeText={v => setCardData({...cardData, name: v})} />
                                    </View>
                                )}
                                {paymentMethod === 'Bank' && (
                                    <View style={styles.cardForm}>
                                ...
                                {paymentMethod === 'Cash' && (
                                    <View style={styles.cardForm}>
                                        <View style={styles.infoBox}>
                                            ...
                                            <Text style={styles.infoTextMain}>Please pay at your local Grama Niladhari office.</Text>
                                        </View>
                            ...
                            <TouchableOpacity style={styles.btnPrimary} onPress={confirmAndSubmit} disabled={submitting}>
                                {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnPrimaryText}>{t.payNow}</Text>}
                            </TouchableOpacity>
                            ...
                            {(applyFor === 'Family' || applyFor === 'Child') && (
                                <View style={styles.familyForm}>
                                    ...
                                    <View style={styles.verifyRow}>
                                        <TextInput style={[styles.input, {flex: 1}, errors.nic && styles.errorBorder]} value={nic} onChangeText={handleNicChange} placeholder="NIC" autoCapitalize="characters" />
                                        <TouchableOpacity style={styles.scanBtnMini} onPress={handleScanNIC}><CameraIcon size={18} color="#fff" /></TouchableOpacity>
                                        {applyFor !== 'Child' && <TouchableOpacity style={styles.verifyBtn} onPress={verifyMember} disabled={verifyingMember}>{verifyingMember ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.verifyBtnText}>Verify</Text>}</TouchableOpacity>}
                                    </View>
                                    <Text style={styles.label}>{t.labels.relationship}</Text>
                                    <TextInput style={[styles.input, errors.relationship && styles.errorBorder]} value={relationship} onChangeText={setRelationship} placeholder="Relationship" />
                                </View>
                            )}
                            <Text style={styles.label}>{t.upload} (Utility Bill)</Text>
                            <TouchableOpacity style={[styles.uploadZone, errors.file && styles.errorBorder]} onPress={pickDocument}>
                                <Upload size={30} color="#800000" />
                                <Text style={styles.uploadTitle}>{selectedFile ? selectedFile.name : 'Choose Utility Bill'}</Text>
                            </TouchableOpacity>

                            <Text style={styles.label}>NIC Image (Front & Back)</Text>
                            <TouchableOpacity style={styles.uploadZone} onPress={async () => {
                                const res = await DocumentPicker.getDocumentAsync({ type: ['image/*'] });
                                if (!res.canceled) setNicFile(res.assets[0]);
                            }}>
                                <CameraIcon size={30} color="#800000" />
                                <Text style={styles.uploadTitle}>{nicFile ? nicFile.name : 'Choose NIC Image'}</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={styles.btnPrimary} onPress={handleApply}><Text style={styles.btnPrimaryText}>{t.proceed}</Text></TouchableOpacity>
                        </View>
                    )
                ) : (
                    <View style={styles.historyList}>
                        {loading ? <ActivityIndicator size="large" color="#800000" /> : (
                            history.length > 0 ? history.map(item => renderApplication({ item })) : <View style={styles.empty}><FileSearch size={50} color="#cbd5e1" /><Text style={styles.emptyText}>{t.empty}</Text></View>
                        )}
                    </View>
                )}
            </ScrollView>
            <Modal visible={showScanner} animationType="slide">
                <View style={styles.scannerContainer}>
                    <CameraView style={styles.camera} facing="back" onCameraReady={onCameraReady} />
                    <View style={styles.overlay}>
                        <View style={styles.scanFrame} />
                        <Text style={styles.scanText}>{scanning ? "Extracting NIC Data..." : "Align NIC inside the frame"}</Text>
                        <TouchableOpacity style={styles.captureBtn} onPress={simulateScan}>
...
const styles = StyleSheet.create({
    ...
    header: { backgroundColor: '#800000', padding: 24, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderBottomLeftRadius: 30, borderBottomRightRadius: 30, elevation: 8 },
    ...
    verifiedText: { color: '#10b981', fontSize: 13, fontWeight: 'bold', marginTop: 5 },
    scanBtnMini: { backgroundColor: '#800000', width: 40, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
    scannerContainer: { flex: 1, backgroundColor: '#000' },
    camera: { flex: 1 },
    overlay: { ...StyleSheet.absoluteFillObject, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' },
    scanFrame: { width: 280, height: 180, borderWidth: 2, borderColor: '#fff', borderRadius: 20 },
    scanText: { color: '#fff', fontSize: 16, fontWeight: 'bold', marginTop: 20, textAlign: 'center', paddingHorizontal: 40 },
    captureBtn: { position: 'absolute', bottom: 50, width: 70, height: 70, borderRadius: 35, backgroundColor: 'rgba(255,255,255,0.3)', justifyContent: 'center', alignItems: 'center' },
