...
export default function SettingsScreen() {
    ...
    const fetchConfig = async () => {
        try {
            ...
            console.warn("⚠️ [SETTINGS] Protected config failed, trying public-config fallback...");
            try {
                // Fallback to public config so the screen at least loads
                const publicRes = await api.get('/system/public-config');
                setConfig(publicRes.data);
            } catch (publicErr) {
                console.error("❌ [SETTINGS] All config fetches failed");
