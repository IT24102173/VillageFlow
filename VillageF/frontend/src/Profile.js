...
import { 
  ...
  Calendar, Mail, Briefcase, Home, FileText, Smartphone
} from 'lucide-react';
...
function Profile() {
    ...
    if (!user) {
        return <div style={{padding: '100px', textAlign: 'center', color: '#800000'}}><Loader2 className="spin" /> Loading Official Profile...</div>;
    }
    ...
    const handleUpdate = async () => {
        ...
        try {
            const targetId = user._id || user.id;
            const res = await axios.put(`http://localhost:5000/api/auth/update/${targetId}`, editData);
    ...
    const handleDelete = async () => {
        if (window.confirm(t.confirmDelete)) {
            try {
                const targetId = user._id || user.id;
                await axios.delete(`http://localhost:5000/api/auth/delete/${targetId}`);
    ...
    const handleProxyRegister = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:5000/api/auth/proxy-register', submitData, {
            ...
            if (err.response) {
                ...
                setProxyErrors({ general: lang === 'si' ? "සේවාදායකයට සම්බන්ධ විය නොහැක. Backend server එක (http://localhost:5000) ක්‍රියාත්මකද පරීක්ෂා කරන්න." : (lang === 'ta' ? "சேவையகத்துடன் இணைக்க முடியவில்லை. Backend server (http://localhost:5000) இயங்குகிறதா சரிபார்க்கவும்." : "Cannot connect to server. Check whether backend server (http://localhost:5000) is running.") });
            } else {
            ...
            <header className="profile-header" style={styles.header}>
                <div className="header-content" style={styles.headerContent}>
                    ...
                    <div className="header-actions" style={styles.headerActions}>
                        <div className="lang-bar" style={styles.langBar}>
                ...
                <div className="profile-container" style={styles.container}>
                    ...
                    <main className="profile-content" style={styles.content}>
                        <section className="profile-section" style={styles.section}>
                            <div className="section-header" style={styles.sectionHeader}><User size={22} color="#800000" /> <h3>{t.profile}</h3></div>
                        ...
                        <section className="proxy-section" style={styles.section}>
                            <div className="section-header" style={styles.sectionHeader}><UserPlus size={22} color="#800000" /> <h3>{t.proxy}</h3></div>
                            
                            <div className="location-box" style={{...styles.locationBox, marginBottom: '20px'}}>
                                <h4 style={{margin: '0 0 10px 0', fontSize: '14px', color: '#800000'}}>
...
const styles = {
    ...
    header: { background: '#800000', color: 'white', padding: '20px 0', borderBottom: '5px solid #fbc531' },
    ...
    govTitle: { fontSize: '16px', fontWeight: '900', color: '#fbc531', margin: 0, letterSpacing: '1px' },
    ...
    activeLang: { fontSize: '13px', fontWeight: 'bold', color: '#fbc531', borderBottom: '2px solid #fbc531' },
    ...
    cardHeader: { background: '#1a1a1a', color: '#fbc531', padding: '12px', fontSize: '11px', fontWeight: 'bold', letterSpacing: '2px' },
    ...
    idRoleBadge: { display: 'inline-block', background: '#800000', color: 'white', padding: '5px 15px', borderRadius: '50px', fontSize: '10px', fontWeight: 'bold', letterSpacing: '1px' },
    ...
    cardFooter: { padding: '10px', background: '#fbc531', color: '#800000', fontSize: '10px', fontWeight: 'bold' },
    ...
    infoIcon: { color: '#800000', marginTop: '4px', opacity: 0.8 },
    ...
    officeCard: { background: 'white', borderRadius: '12px', padding: '20px', borderLeft: '6px solid #800000', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' },
    officeHeader: { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '15px' },
    officeTitle: { fontSize: '14px', fontWeight: 'bold', color: '#800000', margin: 0 },
