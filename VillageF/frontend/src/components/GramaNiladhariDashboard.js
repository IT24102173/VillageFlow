...
import { 
    ...
    TrendingUp, BarChart3, FilterX, ChevronDown, ChevronUp, Bell, Send, Archive, Flag, Award,
...
function GramaNiladhariDashboard() {
    ...
    const [selectedAppForNotify, setSelectedAppForNotify] = useState(null);
    ...
    const fetchPaymentForCertificate = useCallback(async (certificateId) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/certificates/payment/${certificateId}`);
    ...
    const handleElderlyRegister = async (e) => {
        ...
        try {
            ...
            await axios.post('http://localhost:5000/api/auth/proxy-register', payload);
    ...
    const handleUpdateStatus = async (id, status) => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/certificates/update/${id}`, {
                status: status, 
                rejectReason: "", 
    ...
    const submitRejection = async () => {
        try {
            await axios.put(`http://localhost:5000/api/certificates/update/${selectedAppId}`, {
                status: 'Rejected', 
    ...
    const sendNotification = async () => {
        ...
        try {
            await axios.post('http://localhost:5000/api/certificates/notify', {
                applicationId: selectedAppForNotify,
                message: notificationMessage,
                type: 'status_update'
            });
            triggerNotice("දැනුම්දීම සාර්ථකව යවන ලදී.");
            setShowNotificationModal(false);
            setNotificationMessage('');
            setSelectedAppForNotify(null);
        } catch (err) {
            triggerNotice("දැනුම්දීම යැවීම අසාර්ථකයි.", "error");
        }
    };
    ...
    const handleBulkAction = async (action) => {
        ...
        try {
            for (const appId of selectedApplications) {
                if (action === 'approve') {
                    await axios.put(`http://localhost:5000/api/certificates/update/${appId}`, {
                        status: 'Approved',
                        officerName: "Admin_Grama_Niladhari"
                    });
        ...
        <div className="gn-dashboard" style={styles.dashboardWrapper}>
            ...
            <div className="main-content" style={{...styles.mainContent, paddingBottom: isMobile ? '70px' : '0'}}>
                                            ...
                                            <tr key={app._id} style={{...styles.tr, backgroundColor: isUrgent ? '#fef2f2' : 'white'}}>
                                                ...
                                                <td style={styles.td}>
                                                    <div style={styles.actions}>
                                                        ...
                                                        <button onClick={() => {
                                                            setSelectedAppForNotify(app._id);
                                                            setShowNotificationModal(true);
                                                        }} style={styles.notifyBtn} title="Send Notification">
                                                            <Bell size={14}/>
                                                        </button>
                                                        {app.status === 'Pending' && (
                                                            <>
                ...
                {showNotificationModal && (
                    <div style={styles.modalOverlay}>
                        <div style={styles.modalContent}>
                            <h3 style={{color: '#800000', marginBottom:'15px'}}>Send Notification to Applicant</h3>
                            <textarea style={styles.textArea} rows="4" value={notificationMessage} onChange={(e) => setNotificationMessage(e.target.value)} placeholder="Type your notification message here..." />
                            <div style={{display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '20px', flexWrap: 'wrap'}}>
                                <button onClick={() => {
                                    setShowNotificationModal(false);
                                    setNotificationMessage('');
                                ...
                                <button onClick={sendNotification} style={styles.confirmRejectBtn}>
...
const styles = {
    ...
    notifyBtn: { padding: '6px 12px', border: '1px solid #e2e8f0', borderRadius: '8px', cursor: 'pointer', backgroundColor: '#fef3c7', color: '#d97706', display:'flex', alignItems:'center', gap:'5px', fontSize:'12px', fontWeight:'600' },
