...
// React Calendar and its CSS import
...
function AppointmentPage() {
    ...
    const loadConfig = async () => {
        try {
            ...
            if (publicConfig && publicConfig.appointment) {
                ...
                setMaxDays(days);
                const slotsRes = await axios.get('http://localhost:5000/api/appointments/available-slots');
    ...
    const handleUpdateSubmit = async (e) => {
        ...
        try {
            await axios.put(`http://localhost:5000/api/appointments/update/${currentAppointmentId}`, updateFormData);
    ...
    const updateStatus = async (id, newStatus) => {
        try {
            await axios.put(`http://localhost:5000/api/appointments/status/${id}`, { status: newStatus });
    ...
    return (
        <div className="appointment-page" style={styles.container}>
            ...
            <div className="language-selector" style={styles.languageSelector}>
                <button className="lang-btn" onClick={() => setLanguage('sinhala')} style={{...styles.langBtn, backgroundColor: language === 'sinhala' ? '#800000' : '#e2e8f0', color: language === 'sinhala' ? 'white' : '#333'}}>සිංහල</button>
                <button className="lang-btn" onClick={() => setLanguage('english')} style={{...styles.langBtn, backgroundColor: language === 'english' ? '#800000' : '#e2e8f0', color: language === 'english' ? 'white' : '#333'}}>English</button>
                <button className="lang-btn" onClick={() => setLanguage('tamil')} style={{...styles.langBtn, backgroundColor: language === 'tamil' ? '#800000' : '#e2e8f0', color: language === 'tamil' ? 'white' : '#333'}}>தமிழ்</button>
            </div>
            ...
            <div className="header-row" style={styles.headerRow}>
                <h2 className="main-title" style={styles.mainTitle}><Lucide.CalendarCheck color="#800000" size={28} /> {t.appTitle}</h2>
            ...
            <style>
                {`
                    ...
                    .react-calendar__tile--active { background: #800000 !important; color: white !important; border-radius: 6px; }
                    ...
                    .react-calendar__month-view__weekdays__weekday abbr { text-decoration: none; font-weight: bold; color: #800000; }
                    ...
                    input:focus, select:focus { border-color: #800000 !important; box-shadow: 0 0 0 2px rgba(128,0,0,0.1); outline: none; }
...
const styles = {
    ...
    mainTitle: { display: 'flex', alignItems: 'center', gap: '10px', color: '#800000', fontSize: '24px', margin: 0 },
    ...
    bookBtn: { padding: '12px', backgroundColor: '#800000', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontWeight: 'bold', width: '100%', transition: 'all 0.2s' },
    ...
    approveBtn: { background: '#22c55e', color: 'white', border: 'none', padding: '8px', borderRadius: '4px', cursor: 'pointer', transition: 'all 0.2s' },
