...
function WelfarePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await axios.put(`http://localhost:5000/api/welfare/update/${isEditing}`, formData);
                setStatusMessage({ type: 'success', message: 'සාර්ථකව යාවත්කාලීන කළා' });
            } else {
                await axios.post('http://localhost:5000/api/welfare/add', formData);
    ...
    const handleDelete = async (id) => {
        ...
        setSubmitting(true);
        try {
            await axios.delete(`http://localhost:5000/api/welfare/delete/${id}`);
            fetchBeneficiaries();
    ...
    const toggleStatus = async (id, currentStatus) => {
        ...
        setSubmitting(true);
        try {
            await axios.put(`http://localhost:5000/api/welfare/approve/${id}`, { status: newStatus });
            fetchBeneficiaries();
    ...
    const viewPaySlip = (paySlipPath, fullName) => {
        ...
        const fileUrl = `http://localhost:5000/api/welfare/pay-slip/${filename}`;
    ...
    return (
        <div className="welfare-page" style={styles.container}>
            ...
            <div style={styles.statsGrid}>
                <div style={{...styles.statCard, borderTop: '4px solid #800000'}}>
                    <div><FileText size={24} color="#800000" /></div>
            ...
            <div style={styles.tableWrapper}>
                ...
                {loading ? (
                    <div style={styles.loadingContainer}>
                        <Loader size={40} className="spin" color="#800000" />
...
const styles = {
    ...
    pdfBtn: { display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', backgroundColor: '#800000', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '500' },
    ...
    submitBtn: { padding: '14px 28px', backgroundColor: '#800000', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '600', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', transition: 'background 0.2s' },
