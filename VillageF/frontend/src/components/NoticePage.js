...
function NoticePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await axios.put(`http://localhost:5000/api/notices/update/${isEditing}`, formData);
                alert("නිවේදනය යාවත්කාලීන කළා!");
            } else {
                await axios.post('http://localhost:5000/api/notices/add', formData);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("මෙම නිවේදනය මකා දැමීමට අවශ්‍යද?")) {
            await axios.delete(`http://localhost:5000/api/notices/delete/${id}`);
            fetchNotices();
        }
    };
            ...
            <div className="header-bar" style={styles.headerBar}>
                <div>
                    <h2 className="title" style={styles.title}>
                        <Lucide.Megaphone color="#800000" size={32} /> 
            ...
            <div className="notices-grid" style={styles.grid}>
                {notices.map(notice => (
                    <div key={notice._id} className="notice-card" style={{
                        ...
                        borderTopColor: notice.category === 'Emergency' ? '#ef4444' : '#800000'
                    }}>
...
const styles = {
    ...
    addBtn: { backgroundColor: '#800000', color: 'white', border: 'none', padding: '12px 20px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '600', transition: 'all 0.2s ease' },
    ...
    formTitle: { marginTop: 0, color: '#800000', borderBottomWidth: '1px', borderBottomStyle: 'solid', borderBottomColor: '#ffcccc', paddingBottom: '10px' },
    ...
    noticeHeading: { color: '#800000', margin: '0 0 20px 0', fontSize: '20px', lineHeight: '1.3' },
