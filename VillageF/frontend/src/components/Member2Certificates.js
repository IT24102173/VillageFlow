...
// MOCK CARD VALIDATION - For campus project (accepts any 16-digit number)
...
const PaymentModal = ({ amount, certType, onSuccess, onCancel, lang }) => {
    ...
    const validateCardDetails = () => {
        ...
        if (!mockValidateCardNumber(cleanedNumber)) {
            newErrors.cardNumber = 'Enter 15-16 digit card number (Demo: any 16 digits work)';
        }
        ...
        return new Promise((resolve) => {
            setTimeout(() => {
                ...
                setPaymentReceipt({
                    ...
                    cardType: cardTypeDetected || cardDetails.cardType,
    ...
    const processBankTransfer = async () => {
        if (!validateBankDetails()) return false;
    ...
    const processMobilePayment = async () => {
        if (!validateMobileDetails()) return false;
    ...
    const processOfflinePayment = async () => {
        if (!validateOfflinePayment()) return false;
    ...
    const handlePayment = async () => {
        ...
        if (paymentMethod === 'card') {
            ...
            success = await processOfflinePayment();
        }
    ...
    const downloadReceipt = () => {
        ...
        if (paymentReceipt.cardNumber) {
            doc.text(`Card: ****${paymentReceipt.cardNumber}`, 20, 80);
            doc.text(`Card Type: ${paymentReceipt.cardType}`, 20, 87);
        }
        if (paymentReceipt.referenceNumber) {
            doc.text(`Reference: ${paymentReceipt.referenceNumber}`, 20, 80);
        }
        if (paymentReceipt.mobileNumber) {
            doc.text(`Mobile: ****${paymentReceipt.mobileNumber}`, 20, 80);
        }
        if (paymentReceipt.receiptNumber) {
            doc.text(`GN Office Receipt: ${paymentReceipt.receiptNumber}`, 20, 80);
        }
                ...
                <div className="modal-content" style={{...styles.modalContent, maxWidth: '500px'}}>
                    ...
                    <div style={styles.modalBody}>
                        <div style={styles.receiptContainer}>
                            <div style={styles.receiptSuccessIcon}>
                            ...
                            <div style={styles.receiptDetails}>
                                ...
                                <div style={styles.receiptRow}>
                                    <span>{t.certType}:</span>
                                    <strong>{certType}</strong>
                                </div>
                                <div style={styles.receiptRow}>
                                    <span>{t.fee}:</span>
                                    <strong style={{color: '#800000'}}>Rs. {amount}/=</strong>
                                </div>
                                <div style={styles.receiptRow}>
                                    <span>{t.paymentMethod}:</span>
                                    <strong>{paymentReceipt.paymentMethod === 'offline' ? t.offlinePayment : t.paymentMethods[paymentReceipt.paymentMethod] || paymentReceipt.paymentMethod}</strong>
                                </div>
                                <div style={styles.receiptRow}>
                                    <span>{t.paymentDate}:</span>
                                    <strong>{new Date().toLocaleString()}</strong>
                                </div>
                                {paymentReceipt.cardType && (
                                    <div style={styles.receiptRow}>
                                        <span>Card Type:</span>
                                        <strong>{paymentReceipt.cardType}</strong>
                                    </div>
                                )}
                            </div>
                            <button onClick={downloadReceipt} style={styles.downloadReceiptBtn}>
                                <Download size={16} /> {t.downloadReceipt}
                            </button>
                        </div>
                    </div>
                    <div style={styles.modalFooter}>
                        <button onClick={confirmPayment} style={styles.paymentBtn}>
                            <CheckCircle2 size={18} /> {t.continue}
                        </button>
                    </div>
                </div>
            </div>
    ...
    if (paymentStep === 'details') {
        return (
            <div style={styles.modalOverlay}>
                <div className="modal-content" style={{...styles.modalContent, maxWidth: '500px'}}>
                    <div style={styles.modalHeader}>
                        <h3>{paymentMethod === 'card' ? t.cardPayment : paymentMethod === 'bank' ? t.bankPayment : paymentMethod === 'mobile' ? t.mobilePayment : t.offlinePaymentInfo}</h3>
                        <button onClick={() => setPaymentStep('select')} style={styles.modalClose}><X size={20}/></button>
                    </div>
                    <div style={styles.modalBody}>
                        {paymentMethod === 'card' && (
                            <div>
                                {cardTypeDetected && cardDetails.cardNumber.length > 8 && (
                                    <div style={styles.cardTypeIndicator}>
                                        <CreditCard size={20} color="#800000" />
                                        <span>{t.cardTypeDetected}: {cardTypeDetected}</span>
                                        <span style={{marginLeft: 'auto', fontSize: '11px', color: '#666'}}>(Demo Mode)</span>
                                    </div>
                                )}
                                <div style={styles.inputGroup}>
                                    <label style={styles.fieldLabel}>{t.cardNumber}</label>
                                    <input style={{...styles.inputField, borderColor: errors.cardNumber ? '#ef4444' : '#e2e8f0'}} placeholder="1234 5678 9012 3456 (any 16 digits)" value={cardDetails.cardNumber} onChange={(e) => handleCardNumberChange(e.target.value)} maxLength="19" />
                                    {errors.cardNumber && <span style={styles.errorText}>{errors.cardNumber}</span>}
                                    <span style={styles.demoHint}>Demo: Enter any 16-digit number (e.g., 4111111111111111)</span>
                                </div>
                                <div style={styles.inputGroup}>
                                ...
                                <div className="row-two-cols" style={styles.rowTwoCols}>
                                    <div style={styles.inputGroup}>
                                        ...
                                        <input style={{...styles.inputField, borderColor: errors.expiryDate ? '#ef4444' : '#e2e8f0'}} placeholder="MM/YY (e.g., 12/28)" value={cardDetails.expiryDate} onChange={(e) => setCardDetails({...cardDetails, expiryDate: e.target.value})} maxLength="5" />
                        ...
                        {paymentMethod === 'bank' && (
                            <div>
                                <div style={styles.bankInfoBox}>
                                    <Building2 size={20} color="#800000" />
                                    <div>
                                        ...
                                        <p style={{...styles.bankInfoText, color: '#666', fontSize: '11px'}}>Demo: Any account number works</p>
                                    </div>
                                </div>
                                <div style={styles.inputGroup}>
                                ...
                                <div style={styles.inputGroup}>
                                    ...
                                    <input style={{...styles.inputField, borderColor: errors.accountNumber ? '#ef4444' : '#e2e8f0'}} placeholder="123456789012 (any 8-20 digits)" value={bankDetails.accountNumber} onChange={(e) => setBankDetails({...bankDetails, accountNumber: e.target.value})} />
                                    {errors.accountNumber && <span style={styles.errorText}>{errors.accountNumber}</span>}
                                    <span style={styles.demoHint}>Demo: Enter any 10-15 digit number</span>
                                </div>
                                <div style={styles.inputGroup}>
                                    <label style={styles.fieldLabel}>{t.accountHolderName}</label>
                                    <input style={{...styles.inputField, borderColor: errors.accountHolderName ? '#ef4444' : '#e2e8f0'}} placeholder="Enter account holder name" value={bankDetails.accountHolderName} onChange={(e) => setBankDetails({...bankDetails, accountHolderName: e.target.value.toUpperCase()})} />
                        ...
                        {paymentMethod === 'mobile' && (
                            <div>
                                ...
                                <div style={styles.inputGroup}>
                                    ...
                                    <span style={styles.demoHint}>Demo: Enter 07XXXXXXXX format</span>
                                </div>
                                <div style={styles.inputGroup}>
                                    <label style={styles.fieldLabel}>PIN / Password</label>
                                    <input style={{...styles.inputField, borderColor: errors.pin ? '#ef4444' : '#e2e8f0'}} type="password" placeholder="Enter PIN (1234 for demo)" value={mobileDetails.pin} onChange={(e) => setMobileDetails({...mobileDetails, pin: e.target.value})} />
                                    {errors.pin && <span style={styles.errorText}>{errors.pin}</span>}
                                </div>
                                <div style={styles.paymentInfoNote}>
                                    <Smartphone size={14} />
                                    <span>Demo: Any PIN works for demonstration</span>
                                </div>
                            </div>
                        )}
                        ...
                        {paymentMethod === 'offline' && (
                            <div>
                                <div style={styles.offlinePaymentBox}>
                                    <Building2 size={32} color="#800000" />
                                    <div>
                                        <strong>{t.offlinePaymentInfo}</strong>
                                        <p>{t.offlinePaymentDesc}</p>
                                        <p style={{fontSize: '12px', marginTop: '8px', color: '#666'}}>Demo: Enter any receipt number to continue</p>
                                    </div>
                                </div>
                                <div style={styles.inputGroup}>
                                    <label style={styles.fieldLabel}>{t.enterReceiptNo}</label>
                                    <input style={{...styles.inputField, borderColor: errors.offlineReceiptNo ? '#ef4444' : '#e2e8f0'}} placeholder="e.g., GN/RCPT/2024/001 (any value works)" value={offlineReceiptNo} onChange={(e) => setOfflineReceiptNo(e.target.value)} />
        ...
        <div style={styles.modalOverlay}>
            <div className="modal-content" style={styles.modalContent}>
                ...
                <div style={styles.modalBody}>
                    ...
                    <div style={styles.paymentAmount}>
                        <span>{t.fee}:</span>
                        <strong style={{color: '#800000', fontSize: '24px'}}>Rs. {amount}/=</strong>
                    </div>
                    ...
                    <div className="payment-methods-container" style={styles.paymentMethodsContainer}>
                        <label className="payment-method-card" style={styles.paymentMethodCard}>
                            ...
                            <div className="payment-method-icon" style={styles.paymentMethodIcon}><CreditCard size={24} color="#800000" /></div>
                            <div>
                                <strong>{t.cardPayment}</strong>
                                <p>Visa / Mastercard / Amex (Demo Mode)</p>
                            </div>
                        </label>
                        
                        <label className="payment-method-card" style={styles.paymentMethodCard}>
                            <input type="radio" value="bank" checked={paymentMethod === 'bank'} onChange={(e) => setPaymentMethod(e.target.value)} />
                            <div className="payment-method-icon" style={styles.paymentMethodIcon}><Building2 size={24} color="#800000" /></div>
                            <div>
                                <strong>{t.bankPayment}</strong>
                                <p>Direct bank transfer (Demo Mode)</p>
                            </div>
                        </label>
                        
                        <label className="payment-method-card" style={styles.paymentMethodCard}>
                            <input type="radio" value="mobile" checked={paymentMethod === 'mobile'} onChange={(e) => setPaymentMethod(e.target.value)} />
                            <div className="payment-method-icon" style={styles.paymentMethodIcon}><Smartphone size={24} color="#800000" /></div>
                            <div>
                                <strong>{t.mobilePayment}</strong>
                                <p>Dialog / Mobitel / Hutch (Demo Mode)</p>
                            </div>
                        </label>
                        ...
                        <label className="payment-method-card" style={styles.paymentMethodCard}>
                            ...
                            <div className="payment-method-icon" style={styles.paymentMethodIcon}><Building2 size={24} color="#800000" /></div>
                            <div>
                                <strong>{t.offlinePayment}</strong>
                                <p>{t.payAtOffice}</p>
                            </div>
                        </label>
                    </div>
                    ...
                    {paymentMethod === 'bank' && (
                        <div style={styles.bankDetailsDisplay}>
                            <div style={styles.bankDetailsHeader}>
                                <Building2 size={16} /> {t.bankDetails}
                            </div>
                            <div style={styles.bankDetailRow}>
                                <span>Bank:</span>
                                <strong>Peoples Bank</strong>
                            </div>
                            <div style={styles.bankDetailRow}>
                                <span>Account Name:</span>
                                <strong>VillageFlow Digital Services</strong>
                            </div>
                            <div style={styles.bankDetailRow}>
                                <span>Account Number:</span>
                                <strong>123-456-789-001</strong>
                            </div>
                            <div style={styles.bankDetailRow}>
                                <span>Branch:</span>
                    ...
                    {paymentMethod === 'offline' && (
                        <div style={styles.offlineInfoBox}>
                            ...
                            <p>{t.offlinePaymentDesc}</p>
                            <p style={{fontSize: '12px', marginTop: '10px', color: '#800000'}}>
                                <strong>{t.referenceNumber}:</strong> VF{Date.now()}
                            </p>
                        </div>
                    )}
                    ...
                    <div style={styles.demoNote}>
                        ...
                        <span>Demo Mode: Any valid-looking input will work for demonstration purposes.</span>
                    </div>
                </div>
                <div style={styles.modalFooter}>
                    <button onClick={() => setPaymentStep('details')} style={styles.paymentBtn}>
                        <ArrowRight size={18} /> {t.enterPaymentDetails}
                    </button>
    ...
    return (
        <div style={styles.previewContainer}>
            <div style={styles.previewHeader}>
                <Eye size={14} />
                <span>{t.preview}</span>
                <button onClick={onClear} style={styles.previewRemoveBtn}>
                    <X size={14} /> {t.remove}
                </button>
            </div>
...
function Member2Certificates() {
    ...
    const fetchMyApps = useCallback(async () => {
        if (!owner?._id) return;
        try {
            const res = await axios.get(`http://localhost:5000/api/certificates/user/${owner._id}`);
            if (res.data) {
    ...
    const syncOfflineApplications = useCallback(async () => {
        ...
        for (const app of offlineApps) {
            try {
                ...
                await axios.post('http://localhost:5000/api/certificates/apply', formData);
            } catch (err) {
                console.error('Sync failed for:', app.id);
    ...
    const validateFamilyMember = useCallback(async (nicValue) => {
        ...
        try {
            const res = await axios.get(`http://localhost:5000/api/certificates/citizens/check/${nicValue}`);
    ...
    const submitWithPaymentData = async (paymentInfo) => {
        ...
        if (paymentInfo && paymentInfo.transactionId) {
            ...
            formData.append('transactionId', paymentInfo.transactionId);
            formData.append('paymentAmount', paymentInfo.amount);
        } else {
        ...
        try {
            const url = editId 
                ? `http://localhost:5000/api/certificates/update/${editId}` 
                : `http://localhost:5000/api/certificates/apply`;
    ...
    const submitApplication = async () => {
        ...
        if (paymentCompleted && paymentData && paymentData.transactionId) {
            formData.append('paymentStatus', 'completed');
            formData.append('paymentMethod', paymentData.paymentMethod);
            formData.append('transactionId', paymentData.transactionId);
            formData.append('paymentAmount', paymentData.amount);
        } else {
            formData.append('paymentStatus', 'pending');
        }
    ...
    const downloadCertificate = async (app) => {
        const doc = new jsPDF();
        const verifyURL = `http://localhost:3000/verify-cert/${app._id}`;
    ...
    const printCertificate = async (app) => {
        ...
        const qrCode = await QRCode.toDataURL(`http://localhost:3000/verify-cert/${app._id}`);
        
        printWindow.document.write(`
            <html>
                <head>
                    <title>Certificate - ${app.memberName}</title>
                    <style>
                        body { font-family: 'Times New Roman', serif; padding: 40px; }
                        .certificate { border: 2px solid #800000; padding: 30px; max-width: 800px; margin: 0 auto; }
                        .header { text-align: center; border-bottom: 1px solid #800000; margin-bottom: 20px; }
                        .title { color: #800000; font-size: 24px; text-align: center; margin: 20px 0; }
                ...
                <body onload="window.print(); window.close();">
                    <div class="certificate">
                        <div class="header">
                            <h2>DEMOCRATIC SOCIALIST REPUBLIC OF SRI LANKA</h2>
                            <p>DEPARTMENT OF GRAMA NILADHARI ADMINISTRATION</p>
                        </div>
                        ...
                        <div class="footer">
                            ...
                            <p>Authorized Registrar - VillageFlow Digital Authority</p>
                        </div>
                    </div>
                </body>
            </html>
            ...
            <div className="grid" style={styles.grid}>
                <div className="member2-main-card card" style={styles.card}>
                    ...
                    {!ownerVerified && !editId ? (
                        <div className="auth-zone" style={styles.authZone}>
                            <div style={styles.scanIllustration}>
                                <Camera size={40} color="#800000" />
                            </div>
                        ...
                        <form onSubmit={handleApply} style={styles.formContent}>
                            <div className="tab-group" style={styles.tabGroup}>
                                ...
                                <button type="button" onClick={()=>{
                                    ...
                                    setPaymentCompleted(false);
                                    setPaymentData(null);
                                }} className={applyFor==='Family' ? 'tab-active' : 'tab'} style={applyFor==='Family'?styles.tabActive:styles.tab}>
                            ...
                            {showFeeInfo && (
                                <div style={styles.feeInfo}>
                                    ...
                                    <span>{t.fee}: <strong>Rs. {fee}/=</strong></span>
                                </div>
                            )}
                            ...
                            {applyFor === 'Family' && (
                                <div className="family-panel" style={styles.familyPanel}>
                                    <label style={styles.checkRow}>
                                        <input type="checkbox" style={styles.checkbox} checked={isChild} onChange={(e)=>{
                                            ...
                                            if (e.target.checked) {
                                                ...
                                                setMemberExists(false);
                                                setNicValidFormat(false);
                                                if (nic) validateFamilyMember(nic);
                                            }
                                        }} /> {t.child}
                                    </label>
                                    ...
                                    {!isChild && (
                                        <div style={styles.inputWrapper}>
                                            ...
                                            <label style={styles.miniScan}>
                                                <Camera size={18}/>
                                                <input type="file" onChange={(e)=>handleNICScan(e, 'member_check')} style={{display:'none'}}/>
                                            </label>
                                        </div>
                                    )}
                                    ...
                                    {(!isChild && memberExists && memberAge !== null && memberAge >= 18) && (
                                        <div style={styles.memberDetailsCard}>
                                            <div style={styles.memberDetailsHeader}>
                                                <User size={16} color="#800000" />
                                                <span style={{fontWeight: '600'}}>{t.memberDetails}</span>
                                            </div>
                                            <div style={styles.memberDetailRow}>
                                                <span style={{fontWeight: '500'}}>{t.fullName}:</span>
                                                <span>{memberName}</span>
                                            </div>
                                            <div style={styles.memberDetailRow}>
                                                <span style={{fontWeight: '500'}}>{t.ageRestriction}:</span>
                                                <span>{memberAge} years {memberAge >= 65 ? '(Free Fee)' : '(Eligible)'}</span>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <div className="upload-section" style={styles.uploadSection}>
                                ...
                                <div className="upload-strip" style={{...styles.uploadStrip, borderColor: statusMsg.text === t.requiredDocumentMissing ? '#ef4444' : '#cbd5e1', backgroundColor: statusMsg.text === t.requiredDocumentMissing ? '#fef2f2' : 'white'}}>
                                    <Upload size={18} color="#800000" />
                                    <span style={{fontSize:'13px', fontWeight:'500', color: selectedFile ? '#1e293b' : '#94a3b8'}}>
                                        {selectedFile ? selectedFile.name : t.proofAddress}
                                    </span>
                                    <input type="file" onChange={handleFileChange} style={styles.fileOverlay} accept="image/jpeg,image/jpg,image/png,application/pdf" />
                                </div>
                                {fileError && (
                                    <div style={styles.fileErrorMsg}>
                                        <AlertCircle size={12} color="#ef4444" />
                                        <span>{fileError}</span>
                                    </div>
                                )}
                                {statusMsg.text === t.requiredDocumentMissing && (
                                    <div style={styles.requiredDocError}>
                                        <AlertCircle size={12} color="#ef4444" />
                                        <span>{t.requiredDocumentMissing}</span>
                                    </div>
                                )}
                            </div>

                            {selectedFile && (
                                <DocumentPreview file={selectedFile} onClear={() => setSelectedFile(null)} lang={lang} />
                            )}
                ...
                <div className="member2-main-card card" style={styles.card}>
                    <h3 className="card-title" style={styles.cardTitle}>
                        <div style={styles.titleIcon}><Clock size={20} color="white" /></div> 
                        {t.history}
                    </h3>
                                    ...
                                    <div className="app-header" style={styles.appHeader}>
                                        <div>
                                            ...
                                            {app.relationship !== 'Self' && (
                                                <div className="relation-badge" style={styles.relationBadge}>{app.relationship}</div>
                                            )}
                                        </div>
                                    ...
                                    {app.status === 'Rejected' && app.rejectionReason && (
                                        <div style={styles.rejectionNotice}>
                                            ...
                                            <div style={{flex: 1}}>
                                                ...
                                                <button onClick={() => handleResubmit(app)} style={styles.resubmitBtn}>
                                                    <RefreshCw size={12}/> {t.resubmit}
                                                </button>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    <div className="member2-action-row" style={styles.actionRowProfessional}>
                                        {app.status === 'Pending' && (
                                            <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
                                                ...
                                                <button onClick={async() => {if(window.confirm(t.deleteConfirm)) {await axios.delete(`http://localhost:5000/api/certificates/delete/${app._id}`); fetchMyApps();}}} style={styles.deleteBtnProfessional}><Trash2 size={16}/></button>
                                            </div>
                                        )}
                                        {app.status === 'Approved' && (
                                            <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
                                                <button className="member2-primary-action-btn" onClick={()=>downloadCertificate(app)} style={styles.pdfBtnProfessional}>
                                                    <Download size={16}/> {t.downloadCert}
                                                </button>
                                                <button className="member2-secondary-action-btn" onClick={()=>printCertificate(app)} style={styles.printBtnProfessional}>
            ...
            <style>
                {`
                    ...
                    @media (max-width: 768px) {
                        .member2-certificates .header-row {
                            flex-direction: column !important;
                            gap: 15px !important;
                            padding: 15px 20px !important;
                        }
                        .member2-certificates .logo-group {
                            justify-content: center !important;
                        }
                        .member2-certificates .main-title {
                            font-size: 16px !important;
                        }
                        .member2-certificates .member2-main-card {
                            padding: 18px !important;
                            border-radius: 16px !important;
                        }
                        .member2-certificates .member2-action-row {
                            justify-content: stretch !important;
                        }
                        .member2-certificates .member2-action-row > div {
                            width: 100% !important;
                        }
                        .member2-certificates .member2-primary-action-btn,
                        .member2-certificates .member2-secondary-action-btn {
                            flex: 1 1 auto !important;
                            justify-content: center !important;
                        }
                        .member2-certificates .sub-title {
                            font-size: 10px !important;
                        }
                        .member2-certificates .grid {
                            grid-template-columns: 1fr !important;
                            padding: 0 20px !important;
                            gap: 20px !important;
                        }
                        .member2-certificates .card {
                            padding: 20px !important;
                        }
                        .member2-certificates .card-title {
                            font-size: 18px !important;
                            margin-bottom: 20px !important;
                        }
                        .member2-certificates .tab-group {
                            margin-bottom: 20px !important;
                        }
                        .member2-certificates .tab, .member2-certificates .tab-active {
                            padding: 10px !important;
                            font-size: 13px !important;
                        }
                        .member2-certificates .select-field, .member2-certificates .input-field {
                            padding: 12px !important;
                            font-size: 14px !important;
                        }
                        .member2-certificates .family-panel {
                            padding: 15px !important;
                        }
                        .member2-certificates .upload-strip {
                            padding: 12px !important;
                        }
                        .member2-certificates .submit-btn {
                            padding: 14px !important;
                            font-size: 14px !important;
                        }
                        .member2-certificates .history-item-professional {
                            padding: 15px !important;
                        }
                        .member2-certificates .app-header {
                            flex-direction: column !important;
                            gap: 10px !important;
                        }
                        .member2-certificates .action-row-professional {
                            flex-direction: column !important;
                        }
                        .member2-certificates .pdf-btn-professional, 
                        .member2-certificates .print-btn-professional, 
                        .member2-certificates .share-btn-professional,
                        .member2-certificates .edit-btn-professional,
                        .member2-certificates .delete-btn-professional {
                            width: 100% !important;
                            justify-content: center !important;
                            padding: 10px !important;
                        }
                        .member2-certificates .tracker-wrapper {
                            flex-wrap: wrap !important;
                            justify-content: center !important;
                            gap: 10px !important;
                        }
                        .member2-certificates .step-container {
                            flex-direction: row !important;
                            gap: 8px !important;
                        }
                        .member2-certificates .step-line {
                            display: none !important;
                        }
                        .member2-certificates .live-bar {
                            margin: 15px 20px !important;
                            padding: 12px 15px !important;
                            font-size: 12px !important;
                        }
                        .member2-certificates .right-actions {
                            justify-content: center !important;
                        }
                        .member2-certificates .modal-content {
                            width: 95% !important;
                            margin: 10px !important;
                        }
                        .member2-certificates .payment-methods-container {
                            gap: 10px !important;
                        }
                        .member2-certificates .payment-method-card {
                            padding: 12px !important;
                        }
                        .member2-certificates .payment-method-icon {
                            width: 40px !important;
                            height: 40px !important;
                        }
                        .member2-certificates .row-two-cols {
                            grid-template-columns: 1fr !important;
                            gap: 10px !important;
                        }
                    }
                    ...
                    @media (max-width: 480px) {
                        ...
                        .member2-certificates .field-label {
                            font-size: 12px !important;
                        }
                        .member2-certificates .submit-btn {
                            padding: 12px !important;
                        ...
                        .member2-certificates .app-name {
                            font-size: 14px !important;
                        }
                        .member2-certificates .app-meta {
                            font-size: 11px !important;
                        }
                        .member2-certificates .app-date {
                            font-size: 10px !important;
                        }
                        .member2-certificates .relation-badge {
                            font-size: 10px !important;
                        }
                        .member2-certificates .auth-zone {
                            padding: 30px 15px !important;
                        }
                        .member2-certificates .primary-scan-btn {
                    ...
                    /* Hover Effects */
                    .member2-certificates .submit-btn:hover {
                        transform: translateY(-2px);
                        box-shadow: 0 10px 25px rgba(128,0,0,0.3);
                    }
                    .member2-certificates .pdf-btn-professional:hover,
                    .member2-certificates .print-btn-professional:hover,
                    .member2-certificates .share-btn-professional:hover,
                    .member2-certificates .edit-btn-professional:hover,
                    .member2-certificates .delete-btn-professional:hover {
                        transform: translateY(-2px);
                        opacity: 0.9;
                    }
                    .member2-certificates .upload-strip:hover {
...
const styles = {
    ...
    headerContainer: { background: '#800000', marginBottom: '30px' },
    headerRow: { 
        display: 'flex', 
        justifyContent: 'space-between', 
        alignItems: 'center', 
    ...
    titleIcon: { background: '#800000', padding: '8px', borderRadius: '10px', display: 'flex' },
    ...
    primaryScanBtn: { background: '#800000', color: 'white', padding: '16px 32px', borderRadius: '14px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '10px', border: 'none' },
    scanLoader: { marginTop: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', color: '#800000', fontWeight: '600' },
    ...
    tabActive: { flex: 1, padding: '12px', border: 'none', borderRadius: '12px', background: 'white', color: '#800000', fontWeight: '800', boxShadow: '0 4px 12px rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' },
    ...
    miniScan: { position: 'absolute', right: '15px', top: '15px', cursor: 'pointer', color: '#800000' },
    ...
    relationBadge: { fontSize: '11px', color: '#800000', background: '#fff5f5', padding: '2px 8px', borderRadius: '12px', display: 'inline-block', marginTop: '5px' },
    ...
    submitBtn: { width: '100%', padding: '18px', background: 'linear-gradient(135deg, #800000 0%, #a00000 100%)', color: 'white', border: 'none', borderRadius: '16px', cursor: 'pointer', fontWeight: '800', fontSize: '16px', boxShadow: '0 8px 20px rgba(128,0,0,0.2)' },
    ...
    pdfBtnProfessional: { background: '#800000', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '12px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '10px' },
